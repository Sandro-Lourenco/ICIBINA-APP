...
async def get_course_trailer_hls_asset(
    ...
    if (
        ...
        or requested.suffix.lower() not in _HLS_MEDIA_TYPES
    ):
    ...
    if requested.suffix.lower() == ".m3u8":
        content = _rewrite_hls_manifest(content, token)
    return Response(
        content=content,
        media_type=_HLS_MEDIA_TYPES[requested.suffix.lower()],
...
def _rewrite_hls_manifest(content: bytes, token: str) -> bytes:
