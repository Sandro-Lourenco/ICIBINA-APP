...
class SupabaseCourseRepository(CourseRepository):
    ...
    def __init__(self, client: Client):
        self.client = client
        self._public_course_cache: dict[int, tuple[float, list[Course]]] = {}
        self._public_course_cache_lock = asyncio.Lock()
        try:
            configured_ttl = float(os.getenv("PUBLIC_COURSE_CACHE_TTL_SECONDS", "30"))
        except ValueError:
            configured_ttl = 30.0
        self._public_course_cache_ttl = max(5.0, min(configured_ttl, 300.0))

    def _invalidate_public_course_cache(self) -> None:
        self._public_course_cache.clear()
    ...
    async def list_published_versions(self, *, limit: int = 50) -> List[Course]:
        ...
        cached = self._public_course_cache.get(safe_limit)
        now = time.monotonic()
        if cached and cached[0] > now:
            return cached[1]

        async with self._public_course_cache_lock:
            cached = self._public_course_cache.get(safe_limit)
            now = time.monotonic()
            if cached and cached[0] > now:
                return cached[1]
            courses = await self._load_published_versions(safe_limit)
            self._public_course_cache[safe_limit] = (
                time.monotonic() + self._public_course_cache_ttl,
                courses,
            )
            return courses

    async def _load_published_versions(self, safe_limit: int) -> List[Course]:
        courses_query = (
            self.client.table("courses")
            .select("id")
            .eq("status", "published")
    ...
    async def delete(self, course_id: str) -> bool:
        ...
        deleted = len(_response_data(res)) > 0
        if deleted:
            self._invalidate_public_course_cache()
        return deleted
