...
- Validação de tamanho limite de 500 MB no backend
...
Nota sobre upload resumível:
    O cliente usa o protocolo TUS em blocos de 6 MB com retomada automática.
"""
...
# Limite operacional — também configurado no bucket raw-videos.
MAX_FILE_SIZE_BYTES = min(
    int(os.getenv("MAX_VIDEO_UPLOAD_BYTES", str(500 * 1024 * 1024))),
    500 * 1024 * 1024,
)
...
class GenerateLessonUploadUrlUseCase:
    ...
    async def execute(
        ...
        if size_bytes > MAX_FILE_SIZE_BYTES:
            raise ValidationError(
                "O vídeo excede o limite de 500 MB. Comprima o arquivo ou escolha um vídeo menor."
            )
