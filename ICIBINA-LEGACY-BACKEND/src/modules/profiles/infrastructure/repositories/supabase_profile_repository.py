...
class SupabaseProfileRepository(ProfileRepository):
    ...
    async def anonymize_account(self, user_id: str) -> str:
        avatar_bucket = self.client.storage.from_("avatars")
        avatar_entries = await run_sync_io(avatar_bucket.list, user_id)
        avatar_paths = [
        ...
        if avatar_paths:
            await run_sync_io(avatar_bucket.remove, avatar_paths)

        response = await run_sync_io(
            self.client.rpc("anonymize_account", {"p_user_id": user_id}).execute
        )
        request_id = response.data
        if not isinstance(request_id, str) or not request_id:
            raise RuntimeError("Account anonymization did not return a request id")
        return request_id

    async def disable_auth_account(self, user_id: str) -> None:
        await run_sync_io(self.client.auth.admin.delete_user, user_id, True)
