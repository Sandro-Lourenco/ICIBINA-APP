...
class FakeProgressRepository:
    ...
    def merge_event(self, **values: object) -> LessonProgress:
        ...
        previous = self.state
        completed = bool(values["completed"]) or bool(previous.completed if previous else False)
        percentage = max(
...
async def test_replayed_idempotency_key_does_not_apply_second_event() -> None:
    ...
    request = SyncBatchRequest(
        events=[
            progress_event(event_id="first", key="same-key", percentage=40, watched_seconds=400),
            progress_event(event_id="replay", key="same-key", percentage=80, watched_seconds=800),
        ]
    )
    ...
    await use_case.execute(
        ...
        SyncBatchRequest(
            events=[
                ...
                progress_event(
                    event_id="stale", key="key-stale", percentage=10, watched_seconds=100
                ),
            ]
        ),
    )
...
async def test_invalid_progress_is_reported_without_persistence() -> None:
    ...
    response = await use_case.execute("student-1", SyncBatchRequest(events=[event]))
