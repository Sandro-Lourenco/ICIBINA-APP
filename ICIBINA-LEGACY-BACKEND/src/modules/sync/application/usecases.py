...
def _canonical_progress_percentage(*, watched_seconds: int, duration_seconds: int) -> Decimal:
    return (Decimal(watched_seconds) * Decimal(100) / Decimal(duration_seconds)).quantize(
...
class TelemetryAggregationService:
    ...
    async def process_event(self, user_id: str, event: dict) -> bool:
        ...
        return await run_sync_io(
            ...
            user_id=user_id,
            payload=payload,
            occurred_at=datetime.fromtimestamp(event["created_at"], tz=timezone.utc).isoformat(),
            origin="offline_sync",
...
class ProcessLessonProgressEventUseCase:
    ...
    def _execute_sync(self, student_id: str, event: dict) -> LessonProgress:
        ...
        if not self.validator.validate_progress(payload_dict, event.get("created_at", 0)):
    ...
    def _validate_and_calculate(self, payload: ProgressPayload, *, completed: bool) -> Decimal:
        ...
        if payload.progress_percentage is not None and abs(
...
class UpdateLessonProgressUseCase:
    ...
    def execute(self, student_id: str, request: ProgressWriteRequest) -> LessonProgress:
        percentage = ProcessLessonProgressEventUseCase(self.repository)._validate_and_calculate(
...
class ProcessSyncBatchUseCase:
    def __init__(
        ...
        self.progress_use_case = ProcessLessonProgressEventUseCase(progress_repository)
        self.telemetry_service = TelemetryAggregationService(event_repository)

    async def execute(self, student_id: str, request: SyncBatchRequest) -> SyncBatchResponse:
        ...
        for event in request.events:
            ...
            try:
                if event.action in {
                    ...
                    await self.telemetry_service.process_event(student_id, event_dict)
...
class GenerateDownloadTokenUseCase:
    async def execute(self, user_id: str, request: DownloadTokenRequest) -> DownloadTokenResponse:
