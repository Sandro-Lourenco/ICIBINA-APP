...
class StripeCheckoutGateway:
    async def get_status(self, checkout_id: str, expected_owner_id: str) -> CheckoutStatus:
        ...
        try:
            session = stripe.checkout.Session.retrieve(checkout_id, expand=["subscription"])
        except stripe.error.InvalidRequestError as exc:
        ...
        owner_id = str(_value(session, "client_reference_id") or _value(metadata, "user_id", ""))
        ...
        return CheckoutStatus(
            ...
            owner_id=owner_id,
            status=_canonical_checkout_status(session_status, payment_status, subscription_status),
