...
class SupabaseSubscriptionRepository(SubscriptionRepository):
    ...
    async def get_by_student_and_course(
        ...
        return [self._map_row(typing.cast(dict[str, typing.Any], r)) for r in (res.data or [])]

    async def get_by_student(self, student_id: str) -> List[Subscription]:
        query = self.client.table("subscriptions").select("*").eq("student_id", student_id)
        res = await run_sync_io(query.execute)
        return [self._map_row(typing.cast(dict[str, typing.Any], r)) for r in (res.data or [])]

    async def get_by_id(self, subscription_id: str) -> Optional[Subscription]:
        query = (
            self.client.table("subscriptions").select("*").eq("id", subscription_id).maybe_single()
        )
        res = await run_sync_io(query.execute)
