...
class SubmitTaskUseCase:
    ...
    def __init__(self, repository: AssessmentRepository, course_repository: CourseRepository):
    ...
    async def execute(
        ...
        if existing_retry:
            if existing_retry.task_id != task_id:
                raise ConflictError("Esta chave de idempotência já foi usada em outra atividade.")
        ...
        existing_subs = await self.repository.get_user_submissions_for_tasks(user_id, [task_id])
        ...
        if not is_draft:
            if task.task_type in ["multiple_choice", "true_false"]:
                ...
                if (
                    ...
                    score = (
                        Decimal("10.0") if task.passing_score is None else Decimal("10.0")
