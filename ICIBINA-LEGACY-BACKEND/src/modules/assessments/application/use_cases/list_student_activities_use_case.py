...
class ListStudentActivitiesUseCase:
    ...
    async def execute(self, user_id: str) -> list[dict[str, Any]]:
        ...
        checks = await asyncio.gather(
            *(_has_student_access(self.course_repository, user_id, course) for course in courses)
        )
        ...
        for task in tasks:
            ...
            attempts.sort(
                key=lambda item: (
                    item.submitted_at or item.graded_at or datetime.min.replace(tzinfo=timezone.utc)
                ),
                reverse=True,
            )
            ...
            result.append(
                {
                    ...
                    "teacher_name": teacher_by_id.get(course.instructor_id, "Lawrence Academy"),
                    "attempts_used": len([item for item in attempts if item.status != "draft"]),
                    "submission": None
                    if latest is None
                    else {
                        "id": latest.id,
...
async def _has_student_access(repository: CourseRepository, user_id: str, course: Any) -> bool:
