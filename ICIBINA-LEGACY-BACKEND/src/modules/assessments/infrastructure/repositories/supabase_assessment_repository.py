...
class SupabaseAssessmentRepository(AssessmentRepository):
    ...
    async def get_tasks_by_lesson_id(self, lesson_id: str) -> List[Task]:
        ...
        if not res.data:
            return []
        return [self._map_task_row(typing.cast(dict[str, typing.Any], row)) for row in res.data]
    ...
    async def get_user_submissions_for_tasks(
        ...
        if not res.data:
            return []
        return [self._map_row(typing.cast(dict[str, typing.Any], row)) for row in res.data]
    ...
    async def get_tasks_by_course_ids(self, course_ids: List[str]) -> List[Task]:
        ...
        return [
            self._map_task_row(typing.cast(dict[str, typing.Any], row)) for row in res.data or []
        ]
    ...
    async def get_submission_by_id(self, submission_id: str) -> TaskSubmission:
        res = self.client.table("task_submissions").select("*").eq("id", submission_id).execute()
        if not res.data:
    ...
    async def save_task(self, task: Task) -> Task:
        ...
        if task.id:
            ...
            res = self.client.table("tasks").insert(typing.cast(typing.Any, data)).execute()
        if not res.data:
