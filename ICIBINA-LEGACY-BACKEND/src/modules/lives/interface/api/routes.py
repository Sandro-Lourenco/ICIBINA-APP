...
from src.modules.lives.application.use_cases import (
    CreateLiveEventUseCase,
    DeleteLiveEventUseCase,
    ListPublicLiveEventsUseCase,
    ListTeacherLiveEventsUseCase,
...
def _youtube_url(value: str) -> str:
    ...
    if host != "youtu.be" and not (
        parsed.path.startswith("/live/") or (parsed.path == "/watch" and "v=" in parsed.query)
    ):
...
async def create_live_event(
    ...
    return await CreateLiveEventUseCase(repository).execute(
