...
def test_admin_success_without_ownership_check():
    ...
    result = asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "user_id": "admin-uuid", "role": "admin"}))
...
def test_super_admin_success():
    ...
    result = asyncio.run(
        uc.execute(**{**DEFAULT_PARAMS, "user_id": "super-admin-uuid", "role": "super_admin"})
    )
...
def test_valid_mov_content_type():
    ...
    uc, _, _ = make_use_case()
    result = asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "content_type": "video/quicktime"}))
    assert result["job_id"]
...
def test_valid_m4v_content_type():
    ...
    uc, _, _ = make_use_case()
    result = asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "content_type": "video/x-m4v"}))
    assert result["job_id"]
...
def test_file_exactly_2gb_accepted():
    ...
    uc, _, _ = make_use_case()
    result = asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "size_bytes": MAX_FILE_SIZE_BYTES}))
    assert result["job_id"]
...
def test_file_over_2gb_rejected():
    ...
    uc, _, _ = make_use_case()
    with pytest.raises(ValidationError):
        asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "size_bytes": MAX_FILE_SIZE_BYTES + 1}))
...
def test_file_3gb_rejected():
    ...
    uc, _, _ = make_use_case()
    with pytest.raises(ValidationError):
        asyncio.run(uc.execute(**{**DEFAULT_PARAMS, "size_bytes": 3 * 1024 * 1024 * 1024}))
