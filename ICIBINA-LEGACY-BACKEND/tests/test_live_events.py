...
def test_live_event_input_rejects_unsafe_youtube_urls(url):
    with pytest.raises(PydanticValidationError):
        LiveEventInput(
            title="Live segura", scheduled_for=datetime.now(timezone.utc), youtube_url=url
        )
...
async def test_teacher_cannot_update_another_instructors_event():
    ...
    with pytest.raises(AuthorizationError):
        await UpdateLiveEventUseCase(repo).execute(
            "event-1", {"title": "Ataque"}, "teacher-other", "teacher"
        )
