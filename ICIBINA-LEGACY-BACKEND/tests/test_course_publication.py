...
async def test_current_failed_video_job_blocks_publication_with_actionable_issue():
    snapshot = ready_snapshot()
    snapshot["failed_jobs"] = [{"id": "job-1", "lesson_id": "lesson-1", "status": "dead_letter"}]
    repository = AsyncMock()
    repository.get_instructor_id.return_value = "teacher-1"
    repository.get_publication_snapshot.return_value = snapshot
...
async def test_required_lesson_without_hls_blocks_even_when_job_is_missing():
    ...
    assert checklist["ready"] is False
    assert any(item["code"] == "required-video:lesson-1" for item in checklist["issues"])
...
async def test_required_lesson_with_external_video_can_publish():
    ...
    assert not any(item["code"] == "required-video:lesson-1" for item in checklist["issues"])


@pytest.mark.asyncio
async def test_active_current_video_job_blocks_publication():
    snapshot = ready_snapshot()
    snapshot["pending_jobs"] = [{"id": "job-1", "lesson_id": "lesson-1", "status": "transcoding"}]
    repository = AsyncMock()
    repository.get_instructor_id.return_value = "teacher-1"
    repository.get_publication_snapshot.return_value = snapshot
