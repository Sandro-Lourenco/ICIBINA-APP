...
async def test_repository_persists_period_end_cancellation() -> None:
    ...
    subscription = await repository.mark_cancel_at_period_end("subscription-1", canceled_at)
...
def test_stripe_states_are_mapped_to_canonical_contract(
    ...
    assert (
        _canonical_checkout_status(session_status, payment_status, subscription_status) == expected
    )
