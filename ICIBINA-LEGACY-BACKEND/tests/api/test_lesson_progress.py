...
def allow_course_access() -> AsyncMock:
    repository = AsyncMock()
    repository.get_published_by_id.return_value = SimpleNamespace(monthly_price=Decimal("0.00"))
    return repository
...
def test_progress_reads_only_current_student() -> None:
    ...
    app.dependency_overrides[get_course_repository] = allow_course_access
    try:
        response = client.get("/api/v1/offline/progress")
    finally:
...
def test_progress_rejects_mismatched_lesson_id() -> None:
    ...
    try:
        response = client.patch("/api/v1/offline/progress/lesson-1", json=payload)
    finally:
        app.dependency_overrides.clear()
